...
namespace StoreClouding.SandBoxEngine.Client.Communication
{
    class SocketClient
    {
        ...
        private byte[] buffer = new byte[SocketClient.BufferSize];
        ...
        void GetMoreData()
        {
            ClientHandler.BeginReceive(buffer, 0, SocketClient.BufferSize, 0,
        ...
        void MessageReceived(int readingMessageTypeID, byte[] data)
        {
            ...
            //if ping send pong
            if (readingMessageTypeID == (int)Common.SocketMessageTypes.Ping)
            {
                Send((int)Common.SocketMessageTypes.Ping, Common.SocketDefaultMessages.Ping);
                GetNewMessage();
                return;
            }
